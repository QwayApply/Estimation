...
depm = input()
df_master = df.where((df['Major']==depm) & (df['perc']!='-')); df_master=df_master.dropna()

df1 = df.where(df['Major'] == '약학'); df1 = df1.dropna()
df2 = df.where(df['Major'] == '의예'); df2 = df2.dropna()
df3 = df.where((df['Major'] == '수의예') & (df['perc']!='-')); df3 = df3.dropna()
df4 = df.where((df['Major'] == '경제') & (df['perc']!='-')); df4 = df4.dropna()
df5 = df.where((df['Major'] == '경영')); df5 = df5.dropna()
df6 = df.where((df['Major'] == '컴퓨터') & (df['perc']!='-') & (df['Major'] == '소프트웨어')); df6 = df6.dropna()

master = df_master['perc'].to_numpy()

med = df1['perc'].to_numpy()
predoc = df2['perc'].to_numpy()
vet = df3['perc'].to_numpy()
eco = df4['perc'].to_numpy()
bus = df5['perc'].to_numpy()
com = df6['perc'].to_numpy()
